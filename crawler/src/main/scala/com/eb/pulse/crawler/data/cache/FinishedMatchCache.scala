...
import com.eb.pulse.crawler.data.transformer.FinishedMatchToTransformer
...
class FinishedMatchCache(finishedMatchParser: FinishedMatchParser, val httpUtils: HttpUtils) {
  ...
  private val cache: LoadingCache[Long, Match] = CacheBuilder.newBuilder()
    .expireAfterWrite(24, TimeUnit.HOURS)
    .maximumSize(100)
    .build(new CacheLoader[Long, Match]() {
      def load(mathcId: Long): Match = {
        findMatch(mathcId) match {
          case Some(finishedMatch) => finishedMatch
