...
class SendGameDataTaskTest extends BasicWordSuiteTest {
  ...
  "fill game bean with score" should {
    "evaluate normal score" in {
      ...
      val matches = List(
        MatchSeries(1, 1, 1, Some(true), true, 35, new Timestamp(1)),
        MatchSeries(1, 2, 2, Some(false), true, 35, new Timestamp(2)),
        MatchSeries(1, 3, 3, Some(true), true, 35, new Timestamp(3)),
      )
    ...
    "evaluate rehost score" in {
      ...
      val matches = List(
        MatchSeries(1, 1, 1, Some(true), true, 35, new Timestamp(1)),
        MatchSeries(1, 2, 2, Some(false), true, 35, new Timestamp(2)),
        MatchSeries(1, 3, 2, Some(false), true, 35, new Timestamp(3)),
        MatchSeries(1, 4, 3, Some(true), true, 35, new Timestamp(4))
      )
      task.fillWithScore(bean, matches)
      bean.radiantWin shouldEqual 2
      bean.direWin shouldEqual 1
