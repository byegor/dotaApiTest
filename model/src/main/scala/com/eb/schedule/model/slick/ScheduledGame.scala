...
/**
  ...
  */
case class ScheduledGame(id: Int, radiant: Int, dire: Int, leagueId: Int, startDate: Timestamp, status: Byte = 0)

object ScheduledGame {

  class ScheduledGameTable(_tableTag: Tag) extends Table[ScheduledGame](_tableTag, "scheduled_games") {
    def * = (id, radiant, dire, leagueId, startDate, status) <>((ScheduledGame.apply _).tupled, ScheduledGame.unapply)
