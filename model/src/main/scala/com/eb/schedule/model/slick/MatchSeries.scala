...
/**
  ...
  */
case class MatchSeries(scheduledGameId: Int, matchId: Long, gameNumber: Byte, radiantWin:Option[Boolean])

object MatchSeries {

  class MatchSeriesTable(_tableTag: Tag) extends Table[MatchSeries](_tableTag, "match_series") {
    def * = (scheduledGameId, matchId, gameNumber, radiantWin) <>((MatchSeries.apply _).tupled, MatchSeries.unapply)
//    def ? = (Rep.Some(scheduledGameId), Rep.Some(matchId), Rep.Some(gameNumber), Rep.Some(radiantWin)).shaped.<>({ r => import r._; _1.map(_ => MatchSeries.apply(_1.get, _2.get, _3.get, Option(_4.get))) }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
